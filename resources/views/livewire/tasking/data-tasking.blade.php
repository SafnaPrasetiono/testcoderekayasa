<div>
    <div class="overflow-hidden mb-3 rounded-xl border border-blue-600">
        <div class="flex bg-blue-600 text-white py-3 px-4">
            <h3>OPEN</h3>
            <small class="ms-auto rounded-md px-2 py-1 bg-white text-black">{{$dataopen->count()}} Task</small>
        </div>
        <div class="py-3 px-4 z-10">
            @if ($dataopen->count() == 0)
            <div class="text-center py-5">
                <i class="fal fa-box-open fa-3x"></i>
                <p class="font-bold">Tidak Ada Data!</p>
            </div>
            @else
                @foreach ($dataopen as $item)
                        ...
                        <div class="dropdown">
                            ...
                            <div
                                ...
                                <button type="button" wire:click="edit({{ $item->tasking_id }})"
                                    class="w-full text-start hover:text-blue-500 py-0.5">
                                    <i class="fas fa-pencil fs-sm w-8"></i>Edit
                                </button>
                                <button type="button" wire:click="prepareDelete({{ $item->tasking_id }})"
                                    class="w-full text-start hover:text-rose-500 py-0.5">
                                    <i class="fas fa-trash-alt fa-sm w-8"></i>Delete
                                </button>
                            </div>
                        </div>
                    </div>
    ...
    <div class="overflow-hidden mb-3 rounded-xl border border-yellow-500">
        <div class="flex bg-yellow-500 text-white py-3 px-4">
            <h3>Proses</h3>
            <small class="ms-auto rounded-md px-2 py-1 bg-white text-black">{{$dataproses->count()}} Task</small>
        </div>
        <div class="py-3 px-4 z-10">
            @if ($dataproses->count() == 0)
            <div class="text-center py-5">
                <i class="fal fa-box-open fa-3x"></i>
                <p class="font-bold">Tidak Ada Data!</p>
            </div>
    ...
    <div class="overflow-hidden mb-3 rounded-xl border border-green-500">
        <div class="flex bg-green-500 text-white py-3 px-4">
            <h3>Done</h3>
            <small class="ms-auto rounded-md px-2 py-1 bg-white text-black">{{$datadone->count()}} Task</small>
        </div>
        <div class="py-3 px-4 z-10">
            @if ($datadone->count() == 0)
            <div class="text-center py-5">
    ...
    <div class="overflow-hidden mb-3 rounded-xl border border-red-600">
        <div class="flex bg-red-600 text-white py-3 px-4">
            <h3>CANCLE</h3>
            <small class="ms-auto rounded-md px-2 py-1 bg-white text-black">{{$datacancle->count()}} Task</small>
        </div>
        <div class="py-3 px-4 z-10">
            @if ($datacancle->count() == 0)
            <div class="text-center py-5">
                <i class="fal fa-box-open fa-3x"></i>
                <p class="font-bold">Tidak Ada Data!</p>
            </div>
            @else
                @foreach ($datacancle as $item)
                    <div class="flex align-center justify-between mb-2">
                        <div class="leading-5">
                            <p class="font-bold mb-0 lh-1">{{ $item->tasking }}</p>
                            <small>Due Date: {{ $item->date }}</small>
